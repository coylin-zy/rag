...
> 状态：回收站、Token 风控、版本 Diff/回滚和来源治理已部署；批量导入导出完成安全可用的 V1 修复，完整可恢复任务与灾备演练尚未完成（2026-08-23）<br>
...
> 实现状态：当前分支支持最多 500 个 Markdown 的 dry-run/apply、服务端重算计划、外部路径与同步基线、冲突保护，以及包含 `manifest.json`、当前正文和可选历史版本的流式 ZIP 下载。下面描述的 R2 暂存、独立 Transfer Queue、暂停/重试、恢复元数据和 `verify-backup` 是目标架构，尚未实现，不能视为已验收。
...
当前 V1 由 Worker 按对象流式输出不压缩 ZIP，不把全部 R2 正文缓冲进内存；它覆盖活动 Markdown 和可选历史版本。下列目录、恢复元数据和本地验证流程仍是完整灾备任务的目标格式：
...
- 每项功能明确标注实际完成范围，不把未实现的可恢复任务、purge 或备份恢复验证写成已完成。
- 所有迁移在空库和生产数据副本上通过。
